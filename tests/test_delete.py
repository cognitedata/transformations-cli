...
def test_delete_by_id(client: CogniteClient, cli_runner: CliRunner, obj: Dict[str, Optional[str]]) -> None:
    uuid_conf = str(uuid.uuid1())
    created = client.transformations.create(
        Transformation(
    ...
    assert client.transformations.retrieve(created.id) is not None
    ...
    assert client.transformations.retrieve(created.id) is None


def test_delete_by_external_id(client: CogniteClient, cli_runner: CliRunner, obj: Dict[str, Optional[str]]) -> None:
    uuid_conf = str(uuid.uuid1())
    ...
    assert client.transformations.retrieve(external_id=uuid_conf) is not None
    ...
    assert client.transformations.retrieve(external_id=uuid_conf) is None
