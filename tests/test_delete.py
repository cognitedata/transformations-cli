...
@pytest.mark.unit
def test_delete_by_id(exp_client: ExpCogniteClient, cli_runner: CliRunner) -> None:
    ...
    result = cli_runner.invoke(transformations_cli, ["delete", f"--id={created.id}"])
...
@pytest.mark.unit
def test_delete_by_external_id(exp_client: ExpCogniteClient, cli_runner: CliRunner) -> None:
    uuid_conf = str(uuid.uuid1())
    ...
    result = cli_runner.invoke(transformations_cli, ["delete", f"--external-id={uuid_conf}"])
    assert result.exit_code == 0
...
@pytest.mark.unit
def test_delete_by_both_ids(cli_runner: CliRunner) -> None:
    result = cli_runner.invoke(transformations_cli, ["delete", "--external-id=asd", "--id=1"])
    assert result.exit_code == 1
    assert result.exc_info[1].args[0] == "Please only provide one of id and external id."


@pytest.mark.unit
def test_delete_by_no_ids(cli_runner: CliRunner) -> None:
    result = cli_runner.invoke(transformations_cli, ["delete"])
    assert result.exit_code == 1
